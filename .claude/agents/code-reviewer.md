...
advisor (`engine/fetch.py`, `engine/score.py`, `engine/optimize.py`, `engine/weight_scheme_backtest.py`) driven
...
- The scoring formula (`engine/score.py`) and its weight schemes (`engine/weight_scheme_backtest.py`,
...
2. **Reuse / duplication**: logic re-implemented instead of calling the existing function - e.g. a
   ...
   `engine.weight_scheme_backtest`'s `ease_base`, or a new scheme's math forked instead of extending
   `engine/weight_scheme_backtest.py`'s `SCHEMES`/`score_with_scheme`.
...
+ `engine/weight_scheme_backtest.py`), check:
