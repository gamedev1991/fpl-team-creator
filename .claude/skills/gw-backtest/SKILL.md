...
`engine/backtest.py` is the reusable simulator behind this skill — it already implements the
...
**Do not run this until the target gameweek is confirmed finished** — `engine.backtest.check_gameweek_finished(gw)`
...
2. **Identify test squads.** `engine.backtest.TEST_SQUADS` already has the 3 real GW1 squads
   ...
   `{name: [player_id, ...]}` dict and pass it as `squads=` to `engine.backtest.backtest()` — don't
...
3. **Run the backtest.** For each of the 4 named schemes:
   ```
   python engine/backtest.py --gw <n> --scheme <baseline|conservative|aggressive|new_signing_aware>
   ```
