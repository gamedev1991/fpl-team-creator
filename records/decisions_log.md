...
3. **`engine/score.py` + `engine/weight_scheme_backtest.py`'s ownership term** — added after `injury_mult` had
...
`.claude/skills/gw-backtest/SKILL.md` at `engine/weight_scheme_backtest.py` instead of describing
hand-computation.

**Test status:** `pytest tests/ -q` — 42 passed (was 38; +4 from the new `tests/test_score.py`).

**Superseded note (added during the `master` reconciliation):** by the time this branch merged into
`master`, item 2 above had already been independently and more thoroughly fixed there (ownership
was moved out of `score` entirely into a separate `tiebreak` field, never re-added) — so item 2's
