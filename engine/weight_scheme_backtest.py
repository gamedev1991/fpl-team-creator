...
Named `weight_scheme_backtest.py` (not `backtest.py`) because `engine/backtest.py`
is already a different, older module in this project - an out-of-sample
points-vs-xGI correlation study. This one generalizes an earlier, now-superseded
copy of `engine/score.py`'s formula into a configurable "scheme" so any
...
real gameweek results. It never writes to `engine/score.py` itself (see
/score-calibrate for how a scheme becomes live) and predates the xGI-blended
formula `engine/score.py` uses today - treat its numbers as historical.

Usage:
    python engine/weight_scheme_backtest.py --gw 1 --scheme conservative
    python engine/weight_scheme_backtest.py --gw 1 --custom '{"form_mult": 0.95, ...}'
"""
